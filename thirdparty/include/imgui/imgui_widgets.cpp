...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    static int hovered_alpha{ 140 };
    static int held_alpha{ 0 };
    ...
    hovered_alpha = std::clamp( hovered_alpha, 140, 200 );
    held_alpha = std::clamp( held_alpha, 0, 55 );

    ImU32 col = ImColor( 22, 22, 22, hovered_alpha + held_alpha );

    RenderNavHighlight(bb, id);

    RenderFrame(bb.Min - ImVec2 ( 2, 2 ), bb.Max + ImVec2 ( 2, 2 ), col, true, 3.f );
...
bool ImGui::Checkbox(const char* label, bool* v, float oboje_ti_zhenshina )
{
    ImGui::NewLine( );
    SetCursorPosX( oboje_ti_zhenshina );
    ImGuiWindow* window = GetCurrentWindow( );
    if ( window->SkipItems )
        return false;
    ...
    const ImRect check_bb( ImVec2( window->DC.CursorPos.x + 2, window->DC.CursorPos.y + 3 ), window->DC.CursorPos - ImVec2( 5, 4 ) + ImVec2( label_size.y + style.FramePadding.y * 2, label_size.y + style.FramePadding.y * 2 ) );
    ...
    if ( *v )
        it_hover->second = std::clamp( it_hover->second + ( 7.f * ImGui::GetIO( ).DeltaTime * ( hovered ? 1.f : -1.f ) ), 1.0f, 1.f );
    else
        it_hover->second = std::clamp( it_hover->second + ( 7.f * ImGui::GetIO( ).DeltaTime ), 1.0f, 1.f );

    const ImVec4 hover_dis = ImColor( 255, 255, 255, 255 );
    const ImVec4 hover_act = ImColor( 22, 22, 28, 255 );
    ...
    RenderFrame( check_bb.Min - ImVec2( 1, 1 ), check_bb.Max + ImVec2( 1, 1 ), GetColorU32( ImVec4( it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second ) ), true, 2.5f );

    ImGui::PushStyleColor( ImGuiCol_Text, it_text->second );
    if ( label_size.x > 0.0f )
        RenderText( text_bb.GetTL( ) + ImVec2( 8, 0 ) - ImVec2( 0, 1 ), label );
    ImGui::PopStyleColor( );
...
#include <iostream>
bool ImGui::BeginCombo(const char* label, const char* preview_value, int& wtf, ImGuiComboFlags flags)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2 ( 38, 0 ), window->DC.CursorPos + ImVec2( w - 307, label_size.y + style.FramePadding.y * 2.0f ) );
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(value_x2, frame_bb.Max.y), ImColor ( 22, 22, 28, alpha ), style.FrameRounding, ( flags & ImGuiComboFlags_NoArrowButton ) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left ); // vot eto koroche main shtuka na kotoruyu tikat shto bi spisok vilez
 
    RenderFrameBorder(frame_bb.Min, frame_bb.Max, style.FrameRounding);
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(frame_bb.Min + style.FramePadding, ImVec2(value_x2, frame_bb.Max.y), preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - 7, frame_bb.Min.y + style.FramePadding.y), label);
    ...
    bool ret = Begin(name, wtf, NULL, window_flags);
    PopStyleVar();
    if (!ret)
    {
        EndPopup();
...
bool ImGui::Combo(const char* label, int* current_item, int& wtf, int& i_fuck_kids, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ImGuiContext& g = *GImGui;
    ...
    if ( !BeginCombo( label, preview_value, wtf, ImGuiComboFlags_None ) ) {
        i_fuck_kids = -70;
        return false;
    }
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        if (Selectable(item_text, i_fuck_kids, item_selected))
        {
            value_changed = true;
            *current_item = i;
        }
...
bool ImGui::Combo(const char* label, int* current_item, int& wtf, int& i_fuck_kids, const char* const items[], int items_count, int height_in_items)
{
    const bool value_changed = Combo(label, current_item, wtf, i_fuck_kids, Items_ArrayGetter, (void*)items, items_count, height_in_items);
    return value_changed;
}

// Combo box helper allowing to pass all items in a single string literal holding multiple zero-terminated items "item1\0item2\0"
bool ImGui::Combo(const char* label, int* current_item, int& wtf, int& i_fuck_kids, const char* items_separated_by_zeros, int height_in_items)
{
    int items_count = 0;
    ...
    bool value_changed = Combo(label, current_item, wtf, i_fuck_kids, Items_SingleStringGetter, (void*)items_separated_by_zeros, items_count, height_in_items);
    return value_changed;
}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float size = 220.f;
    const ImRect rect( window->DC.CursorPos, window->DC.CursorPos + ImVec2( size, 36.f ) );
    const ImRect clickable( window->DC.CursorPos + ImVec2( 0.f, 17.f ), window->DC.CursorPos + ImVec2( size, 36.f ) );
    ...
    const bool hovered = ItemHoverable( clickable, id );
    bool temp_input_is_active = temp_input_allowed && TempInputIsActive( id );
    if ( !temp_input_is_active )
    {
        const bool focus_requested = temp_input_allowed && FocusableItemRegister( window, id );
    ...
    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior( clickable, id, data_type, p_data, p_min, p_max, format, flags, &grab_bb );
    if ( value_changed )
        MarkItemEdited( id );
    ...
    window->DrawList->AddRectFilled( clickable.Min + ImVec2( 0.f, 7.f ), clickable.Max - ImVec2( 0.f, 7.f ), ImColor( 28, 29, 32, static_cast < int > ( style.Alpha * 255 ) ) );

    window->DrawList->AddRectFilled( clickable.Min + ImVec2( 0.f, 7.f ), grab_bb.Max - ImVec2( 10.f, 5.f ), ImColor( 153, 153, 153, static_cast < int > ( style.Alpha * 255 ) ) );

    window->DrawList->AddCircleFilled( grab_bb.Min + ImVec2( 5.f, 7.f ), 6.f + it_anim->second.circle_anim, ImColor( 255, 255, 255, static_cast < int > ( style.Alpha * 255 ) ), 30.f );

    window->DrawList->AddText( rect.Min + ImVec2( 0, 5 ), ImColor( 255, 255, 255, static_cast < int > ( style.Alpha * 255 ) ), label );

    PushStyleColor( ImGuiCol_Text, ImVec4( ImColor( 255, 255, 255, static_cast < int > ( style.Alpha * 255 ) ) ) );
    RenderTextClipped( rect.Min + ImVec2 ( 0, 5 ), rect.Max - ImVec2 ( 1.f, 0 ), value_buf, value_buf_end, NULL, ImVec2( 1.f, 0.f ) );
    PopStyleColor( );
    ...
    if ( temp_input_is_active )
    {
        ...
        const bool is_clamp_input = true;
        return TempInputScalar( frame_bb, id, label, data_type, p_data, format, is_clamp_input ? p_min : NULL, is_clamp_input ? p_max : NULL );
    }
    ...
    if ( grab_bb.Max.x > grab_bb.Min.x )
        RenderFrame( frame_bb.Min, ImVec2( grab_bb.Max.x, frame_bb.Max.y ), ImColor ( 255, 255, 255, 255 ), true, g.Style.FrameRounding );
    ...
    if ( value_size.x > 0.0f && it_padding->second > 0.f )
    {
        ...
        Begin( window_name, 0, NULL, flags );
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if ( flags & ImGuiColorEditFlags_PickerHueBar ) {
        ...
        if ( alpha_bar ) {
            ...
            Button( "alpha", ImVec2( bars_width, sv_picker_size ) );
            if ( IsItemActive( ) ) {
    ...
    if ( alpha_bar ) {
        ...
        draw_list->AddRectFilledMultiColor( bar1_bb.Min, bar1_bb.Max, 255, 255, 255 & ~IM_COL32_A_MASK, 255 & ~IM_COL32_A_MASK );
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_picker)
    {
        ...
        for (int picker_type = 0; picker_type < 2; picker_type++)
        {
            ...
            if (Selectable("##selectable", 0, false, 0, picker_size)) // By default, Selectable() is closing popup
...
bool ImGui::Selectable(const char* label, int wtfff, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (hovered || selected)
    {
        ...
        //RenderFrame(bb_enlarged.Min, bb_enlarged.Max, col, false, 0.0f); eto koroche vot zalupa hovered mojno potom rework sdelat`
    ...
    if (flags & ImGuiSelectableFlags_Disabled) PushStyleColor(ImGuiCol_Text, style.Colors[ImGuiCol_TextDisabled]); // syda vot toje animation dobavit` daaaaa

    static int hovered_alpha{};
    static int selected_alpha{};
    if ( hovered ) { // tut mojno animation sdelat` kogda najimaesh
        hovered_alpha += 70;
    }
    else
        hovered_alpha = 0;
    ...
    if ( hovered_alpha > 100 )
        hovered_alpha = 100;

    if ( selected_alpha > 100 )
        selected_alpha = 100;

    window->DrawList->AddText( text_min + ImVec2 ( 0, wtfff ), ImColor( 255, 255, 255, 165 + hovered_alpha + selected_alpha ), label );
...
bool ImGui::Selectable(const char* label, int wtfff, bool* p_selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    if (Selectable(label, wtfff, *p_selected, flags, size_arg))
    {
...
bool ImGui::BeginMainMenuBar()
{
    ...
    bool is_open = Begin("##MainMenuBar", 64, NULL, window_flags) && BeginMenuBar();
    PopStyleVar(2);
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        pressed = Selectable(label, 0, menu_is_open, ImGuiSelectableFlags_NoHoldingActiveID | ImGuiSelectableFlags_SelectOnClick | ImGuiSelectableFlags_DontClosePopups | (!enabled ? ImGuiSelectableFlags_Disabled : 0), ImVec2(w, 0.0f));
        PopStyleVar();
    ...
    else
    {
        ...
        pressed = Selectable(label, 0, menu_is_open, ImGuiSelectableFlags_NoHoldingActiveID | ImGuiSelectableFlags_SelectOnClick | ImGuiSelectableFlags_DontClosePopups | ImGuiSelectableFlags_SpanAvailWidth | (!enabled ? ImGuiSelectableFlags_Disabled : 0), ImVec2(min_w, 0.0f));
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        window->DC.CursorPos.x += IM_FLOOR(style.ItemSpacing.x * 0.5f);
        PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(style.ItemSpacing.x * 2.0f, style.ItemSpacing.y));
        pressed = Selectable(label, 0, false, flags, ImVec2(w, 0.0f));
        PopStyleVar();
        window->DC.CursorPos.x += IM_FLOOR(style.ItemSpacing.x * (-1.0f + 0.5f)); // -1 spacing to compensate the spacing added when Selectable() did a SameLine(). It would also work to call SameLine() ourselves after the PopStyleVar().
    }
    else
    {
        ...
        float extra_w = ImMax(0.0f, GetContentRegionAvail().x - min_w);
        pressed = Selectable(label, 0, false, flags | ImGuiSelectableFlags_SpanAvailWidth, ImVec2(min_w, 0.0f));
        if (shortcut_w > 0.0f)
        {
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
    static int wtf{ 64 };
    bool open = BeginCombo("##v", NULL, wtf, ImGuiComboFlags_NoPreview | ImGuiComboFlags_HeightLargest);
    PopStyleColor(2);
