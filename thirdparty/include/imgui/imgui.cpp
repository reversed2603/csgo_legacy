...
static void             RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size, int who_cares );
static void             RenderWindowTitleBarContents(ImGuiWindow* window, const ImRect& title_bar_rect, const char* name, bool* p_open);
static void             RenderWindowDecorationsForColorPicker( ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[ 4 ], float resize_grip_draw_size, int who_cares );
...
bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    bool ret = Begin(title, 64, NULL, flags);
...
static void ImGui::RenderWindowOuterBorders(ImGuiWindow* window)
{
    ...
    if (border_size > 0.0f && !(window->Flags & ImGuiWindowFlags_NoBackground))
        window->DrawList->AddRect(window->Pos, window->Pos + window->Size - ImVec2( 299, 0 ), GetColorU32( ImGuiCol_Border ), rounding, ImDrawCornerFlags_All, border_size );
...
void ImGui::RenderWindowDecorationsForColorPicker( ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[ 4 ], float resize_grip_draw_size, int who_cares ) {
    ImGuiContext& g = *GImGui;
    ImGuiStyle& style = g.Style;
    ImGuiWindowFlags flags = window->Flags;
    ...
    else
    {
        ...
        if ( !( flags & ImGuiWindowFlags_NoBackground ) )
        {
            ...
            window->DrawList->AddRectFilled( window->Pos + ImVec2( 140, 0 ), window->Pos + window->Size - ImVec2( 250, who_cares ), bg_col, window_rounding, ( flags & ImGuiWindowFlags_NoTitleBar ) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Bot );
        }
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size, int who_cares )
{
    ImGuiContext& g = *GImGui;
    ImGuiStyle& style = g.Style;
    ImGuiWindowFlags flags = window->Flags;
    ...
    else
    {
        ...
        if (!(flags & ImGuiWindowFlags_NoBackground))
        {
            ...
            window->DrawList->AddRectFilled(window->Pos - ImVec2(0, 0 ), window->Pos + window->Size - ImVec2 ( 364, who_cares ), ImColor ( 17, 17, 17, whotf ), window_rounding, ( flags & ImGuiWindowFlags_NoTitleBar ) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Bot );
        }
...
bool ImGui::BeginForFuckingColorPicker( const char* name, int who_cares, bool* p_open, ImGuiWindowFlags flags )
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    ...
    if ( first_begin_of_the_frame )
    {
        ...
        // FIXME: More code may rely on explicit sorting of overlapping child window and would need to disable this somehow. Please get in contact if you are affected.
        {
            ...
            const bool title_bar_is_highlight = want_focus || ( window_to_highlight && window->RootWindowForTitleBarHighlight == window_to_highlight->RootWindowForTitleBarHighlight ); // krch tut zavtra alphu menyat ili animation koroche ya ishak
           // RenderWindowDecorationsForColorPicker( window, title_bar_rect, title_bar_is_highlight, resize_grip_count, resize_grip_col, resize_grip_draw_size, who_cares );
...
bool ImGui::Begin(const char* name, int who_cares, bool* p_open, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    IM_ASSERT(name != NULL && name[0] != '\0');     // Window name required
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // FIXME: More code may rely on explicit sorting of overlapping child window and would need to disable this somehow. Please get in contact if you are affected.
        {
            ...
            const ImGuiWindow* window_to_highlight = g.NavWindowingTarget ? g.NavWindowingTarget : g.NavWindow;
            const bool title_bar_is_highlight = want_focus || (window_to_highlight && window->RootWindowForTitleBarHighlight == window_to_highlight->RootWindowForTitleBarHighlight); // krch tut zavtra alphu menyat ili animation koroche ya ishak
            RenderWindowDecorations(window, title_bar_rect, title_bar_is_highlight, resize_grip_count, resize_grip_col, resize_grip_draw_size, who_cares );
...
static void ImGui::ErrorCheckBeginEndCompareStacksSize(ImGuiWindow* window, bool write)
{
    ...
    { int n = g.StyleModifiers.Size;      if ( write ) *p = ( short ) n; else {} }    // Too few or too many PopStyleVar()
...
void ImGui::BeginTooltipEx(ImGuiWindowFlags extra_flags, ImGuiTooltipFlags tooltip_flags)
{
    ...
    Begin(window_name, 64, NULL, flags | extra_flags);
}
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags flags)
{
    ...
    bool is_open = BeginForFuckingColorPicker(name, 64, NULL, flags);
...
bool ImGui::BeginPopup(const char* str_id, ImGuiWindowFlags flags)
{
    ...
    flags |= ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize;
...
bool ImGui::BeginPopupModal(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    const bool is_open = Begin(name, 64, p_open, flags);
...
void ImGui::NavUpdateWindowingOverlay()
{
    ...
    Begin("###NavWindowingList", 64, NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings);
...
{
    if (!ImGui::Begin("Dear ImGui Metrics", 64, p_open))
    {
        ImGui::End();
        return;
    }
    ...
    if (ImGui::TreeNode("Tools"))
    {
        ...
        static int who{ 64 };
        static int wtf{ -70 };
        show_windows_rects |= ImGui::Combo("##show_windows_rect_type", &show_windows_rect_type, who, wtf, wrt_rects_names, WRT_Count, WRT_Count);
