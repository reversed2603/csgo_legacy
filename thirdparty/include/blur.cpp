...
inline void SetDevice(IDirect3DDevice9* setdevice)
{
    device = setdevice;
}

inline void DrawRectBlur(ImDrawList* drawList, const ImVec2& p_min, const ImVec2& p_max, int rounding)
{
    drawList->AddCallback(BeginBlur, device);

    for (int i = 0; i < 8; ++i) {
        drawList->AddCallback(FirstBlurPass, device);
        drawList->AddImage(blurTexture, p_min, p_max, { 1.0f, 1.0f });
        drawList->AddCallback(SecondBlurPass, device);
        drawList->AddImage(blurTexture, p_min, p_max, { 1.0f, 1.0f });
    }

    drawList->AddCallback(EndBlur, device);
    drawList->AddImageRounded(blurTexture, p_min, p_max, ImVec2(backbufferWidth * 1, backbufferHeight * 1), ImVec2(backbufferWidth * 1, backbufferHeight * 1), ImColor(255, 255, 255, 255), rounding);
}
