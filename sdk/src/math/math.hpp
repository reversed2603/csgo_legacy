...
namespace sdk {
	...
	struct vec3_t_ {
		...
		ALWAYS_INLINE vec3_t_& operator/= ( float value )
		{
			x /= value;
			y /= value;
			z /= value;
		...
		ALWAYS_INLINE vec3_t_& normalize( )
		{
		...
		ALWAYS_INLINE vec3_t_ normalized( ) const
		{
			auto ret = *this;
		...
		ALWAYS_INLINE bool operator== ( const vec3_t_& value ) const { return x == value.x && y == value.y && z == value.z; }

		ALWAYS_INLINE bool operator!= ( const vec3_t_& value ) const { return !( operator== ( value ) ); }

		ALWAYS_INLINE float& operator[]( int i ) { return reinterpret_cast< float* >( this )[ i ]; }

		ALWAYS_INLINE const float operator[]( int i ) const { return reinterpret_cast< const float* >( this )[ i ]; }
		...
		ALWAYS_INLINE float dot_product( const vec3_t_& value ) const { return x * value.x + y * value.y + z * value.z; }

		ALWAYS_INLINE float dot_product( const float* value ) const { return x * value[ 0 ] + y * value[ 1 ] + z * value[ 2 ]; }

		ALWAYS_INLINE vec3_t_ cross_product( const vec3_t_& value ) const { return vec3_t_( y * value.z - z * value.y, z * value.x - x * value.z, x * value.y - y * value.x ); }

		ALWAYS_INLINE float dist_to( const vec3_t_& value ) const { return ( *this - value ).length( ); }

		ALWAYS_INLINE float dist_to_2d( const vec3_t_& value ) const { return ( *this - value ).length_2d( ); }

		ALWAYS_INLINE bool is_valid( ) const { return std::isfinite( x ) && std::isfinite( y ) && std::isfinite( z ); }

		ALWAYS_INLINE float distance_squared( const vec3_t_& v ) const
		{
			return ( ( *this - v ).length_sqr( ) );
		}

		ALWAYS_INLINE bool is_zero( )
		{
		...
		ALWAYS_INLINE void normalize_in_place( )
		{
			vec3_t_& v = *this;
	...
	struct vec4_t : public vec3_t_ {
		...
		ALWAYS_INLINE vec4_t& operator= ( const vec3_t_& value )
		{
			x = value.x;
			y = value.y;
			z = value.z;
